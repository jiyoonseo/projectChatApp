...
mongoose.connect('mongodb://localhost/newChat', function(err){
	if(err){
		console.log(err);
...
io.sockets.on('connection', function(socket){
	...
	socket.on('re-reload', function(docs){
		Chat.find({}, function(err, docs){
			if(err) throw err;
			socket.emit('loading again', docs);
		})
	});

	//delete msgs from mongoDB
	socket.on('delete all msgs', function(data, callback){
		Chat.remove({userid: data}, function(err){
			if(err){
				console.log(err);
			}else{
				console.log('good! DELETED BY', data);
		...
		if(data.substring(0,3) === '/p '){ //private message
			...
			if(privateMsgTo in users){
				io.sockets.emit('private message', {msg: trimmedMsg,
												 ...
												 recepientInRoom: true, // *mark
												 currentT: date.toLocaleTimeString()
												});  //send including me
			}else{
				io.sockets.emit('private message', {msg: trimmedMsg,
												 userid: socket.nickname,
												 recepient: privateMsgTo,
												 recepientInRoom: false, // *mark
												 currentT: date.toLocaleTimeString()
												});  //send including me
			}
